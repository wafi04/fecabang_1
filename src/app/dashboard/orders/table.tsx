...
import { TransactionsAllData } from "@/shared/types/transaction";
          ...
          {data.length === 0 ? (
            ...
            data.map((transaction) => (
              <TableRow key={transaction.id} className="border-b">
                <TableCell className="font-medium border-r">{transaction.id}</TableCell>
                <TableCell className="font-mono text-sm border-r">
                  {transaction.referenceId}
                </TableCell>
                <TableCell className="border-r">{transaction.productName}</TableCell>
                <TableCell className="border-r">{transaction.tujuan}</TableCell>
                <TableCell className="border-r">{FormatCurrency(transaction.price)}</TableCell>
                <TableCell className="border-r">{FormatCurrency(transaction.fee)}</TableCell>
                <TableCell className="font-semibold border-r">
                  {FormatCurrency(transaction.total)}
                </TableCell>
                <TableCell
                  className={`border-r ${
                    transaction.laba > 0 ? "text-green-600" : "text-red-600"
                  }`}
                >
                  {FormatCurrency(transaction.laba)}
                </TableCell>
                <TableCell className="border-r">{transaction.payment_Name}</TableCell>
                <TableCell className="border-r">{transaction.username}</TableCell>
                <TableCell className="border-r">{getStatusBadge(transaction.status)}</TableCell>
