...
export const useOrderStore = create<OrderState>((set, get) => ({
  ...
  getCalculation: () => {
    ...
    let methodFee = 0;
    if (selectedMethod) {
      switch (selectedMethod.calculationType) {
        case "fixed":
          methodFee = selectedMethod.feeAmount;
          break;
        case "percentage":
          methodFee = (productPrice * selectedMethod.feePercentage) / 100;
          break;
        case "hybrid":
          methodFee =
            selectedMethod.feeAmount +
            (productPrice * selectedMethod.feePercentage) / 100;
          break;
      }
    }
    ...
    return {
      ...
      methodFee,
      voucherDiscount,
      totalAmount: Math.max(totalAmount, 0), // jangan sampai negatif
    };
  },
