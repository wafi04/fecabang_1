...
export function TableFees({ referenceId }: TableFeesProps) {
  ...
  const [editedFees, setEditedFees] = useState<Record<number, Fees>>({});

  // sync data dari server ke local state
  useEffect(() => {
    if (data?.data) {
      const mapped: Record<number, Fees> = {};
  ...
  const handleChange = (id: number, field: string, value: number | string | boolean) => {
    setEditedFees((prev) => ({
      ...prev,
      [id]: {
        ...prev[id],
                ...
                <TableRow key={fee.id}>
                  ...
                  <TableCell>
                    <Select
                      ...
                      onValueChange={(val) =>
                        handleChange(fee.id, "calculationType", val)
                      }
                    >
